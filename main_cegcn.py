...
def train_model(treeDic, x_test, x_train, args, iter):
    model = CeGCN(args).to(args.device)

    PL_params = list(map(id, model.CommunityGCN.conv1.parameters()))
    TD_params = list(map(id, model.TDrumorGCN.conv1.parameters()))
    base_params = filter(lambda p: id(p) not in PL_params+TD_params, model.parameters())
    optimizer = torch.optim.Adam([
        {'params': base_params},
        {'params': model.CommunityGCN.conv1.parameters(), 'lr': args.lr / args.lr_scale_td},
        {'params': model.TDrumorGCN.conv1.parameters(), 'lr': args.lr / args.lr_scale_bu},
    ...
    for epoch in range(args.n_epochs):
        ...
        early_stopping(np.mean(temp_val_losses), np.mean(temp_val_accs), np.mean(temp_val_F1), np.mean(temp_val_F2),
                       np.mean(temp_val_F3), np.mean(temp_val_F4), model, 'CeGCN', args.datasetname)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasetname', type=str, default="Twitter15", metavar='dataname', help='dataset name')
    parser.add_argument('--input_features', type=int, default=5000, metavar='inputF',
                        help='dimension of input features (TF-IDF)')
    parser.add_argument('--hidden_features', type=int, default=64, metavar='hidden_features',
                        help='dimension of graph hidden state')
    parser.add_argument('--output_features', type=int, default=128, metavar='output_features',
    ...
    parser.add_argument('--num_cuda', type=int, default=0, help='index of GPU')
    ...
    parser.add_argument('--iterations', type=int, default=5, metavar='F',
    ...
    parser.add_argument('--BUdroprate', type=float, default=0., metavar='TDdroprate',
                        help='drop rate for edges in the bottom-up dispersion graph')
    parser.add_argument('--exp_name', type=str, default='CeGCN_Twitter15',
                        help='experiment name for visdom')
